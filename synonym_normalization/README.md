# synonym_normalization/ — Этап 2: Синонимы и алиасы

Постановка проблемы + решение для обработки синонимии сущностей, написанное
после того, как проблема неоднократно всплывала во время ручной разметки NER/RE
(см. `../ner_re_extraction/`). **Реализовано** и встроено в `../scripts/ingest.py`
(`normalize_entities` / `normalize_relations` вызываются на каждом чанке после
Natasha + LLM, до слияния в граф), согласно требованию задания хакатона
...
Модули: `synonym_dictionary.py` (`SynonymDictionary` — `resolve`/`add_term`/
`load`/`save`, один канонический ключ на понятие со списком алиасов),
`canonicalizer.py` (нормализация поверхностной формы к ключу),
`entity_matcher.py` (`deduplicate_entities`), `units_normalization.py`
(`normalize_unit` для атрибутов `Property`), `normalize_pipeline.py`
(оркестрация поверх `EnrichedEntity`/`EnrichedRelation` из
`../llm_pipeline_fewshot/models.py`).
